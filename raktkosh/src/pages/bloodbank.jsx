...
const BloodBankPage = props => {
  ...
  const handleDelete = id => {
    ...
    axios.delete(`/bloodbank/${id}`)
      .then(res => {
        handleResponse(res => ({ ...res, severity: "success", message: "Blood Bank Deleted Successfully" }));
      })
      .catch(err => {
        handleResponse(res => ({ ...res, severity: "error", message: "Failed To Delete Blood Bank" }));
      })
      .finally(() => {
      ...
      <TableContainer component={Paper} className={classes.container}>
        <Typography component="h1" className={classes.title}>
          Registerd Blood Banks
          {
            ...
            <Fab variant="extended" color="primary" className={classes.add_btn} onClick={() => handleOpen(!false)}>
              <Add />
              Add Blood Bank
            </Fab>
          }
              ...
              banks.map((row) => (
                <TableRow key={row.regID}>
                  <TableCell component="th" scope="row">
                    <Link to={`/bloodbank/${row.id}`} component={RLink}>{row.regID}</Link>
                  </TableCell>
